...
@Injectable()
export class ApiInviteUserService {
  private readonly logger = new Logger(ApiInviteUserService.name)
  ...
  async userGetInvite(userId: string) {
    ...
    return this.core.data.user
      .findUnique({
        where: { id: userId },
        include: { usedInvite: { include: { users: true, owner: true } } },
      })
      .then((res) =>
        res?.usedInvite
          ? {
  ...
  async userAcceptInvite(userId: string, code: string) {
    ...
    await this.core.getUserById(userId)
