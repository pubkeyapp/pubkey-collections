import {
  ...
  UserRole,
  UserStatus,
} from '@pubkey-collections/sdk'
import { getUserCookie, sdk, uniqueId } from '../support'

function avatarUrl(name) {
  return `https://source.boringavatars.com/beam/120/pubkey-${name}`
}
...
describe('api-user-feature', () => {
  describe('api-user-admin-resolver', () => {
    describe('authorized', () => {
      ...
      beforeAll(async () => {
        cookie = await getUserCookie('alice', 'password')
      })
      ...
      it('should create a user', async () => {
        ...
        expect(user.allowDm).toBe(false)
      ...
      it('should update a user', async () => {
        ...
        expect(user.allowDm).toBe(input.allowDm)
        expect(user.avatarUrl).toBe(input.avatarUrl)
        expect(user.language).toBe(input.language)
        expect(user.location).toBe(input.location)
        expect(user.name).toBe(input.name)
        expect(user.role).toBe(input.role)
        expect(user.status).toBe(input.status)
        expect(user.verified).toBe(input.verified)
      })
    ...
    describe('unauthorized', () => {
      let cookie: string
      beforeAll(async () => {
        cookie = await getUserCookie('bob', 'password')
      })
